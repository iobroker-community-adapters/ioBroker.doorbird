...
=======
	...
	async onReady() {
		...
		this.birdpw = this.config.birdpw ? this.encrypt(this.config.birdpw) : '';
		this.log.debug(`Entcrypted Password: ${this.birdpw}`);
		await Promise.all([this.migrateAsync(), this.mainAsync()]);
	}
					...
					if (res.socket && res.socket.remoteAddress) {
						...
						if (remoteAddress === this.config.birdip || this.config.listenOnAllInterfaces) {
							...
							if (req.url == '/motion') {
								...
								this.setTimeout(async () => {
									await this.setState('Motion.trigger', false, true);
								}, 2500);
							}
							if (req.url && req.url.indexOf('ring') != -1) {
								const id = req.url.substring(req.url.indexOf('?') + 1, req.url.length);
								...
								this.setTimeout(async () => {
									await this.setState('Doorbell.' + id + '.trigger', false, true);
								}, 2000);
							}
							res.end();
						}
	...
	async getInfoAsync() {
		if (this.authorized) {
			try {
				...
				if (response.status === 200) {
					const info = response.data;
					await this.setState('info.firmware', info.BHA.VERSION[0].FIRMWARE, true);
					await this.setState('info.build', info.BHA.VERSION[0].BUILD_NUMBER, true);
					await this.setState('info.type', info.BHA.VERSION[0]['DEVICE-TYPE'], true);
					...
					if (obj.value.includes(this.config.adapterAddress + ':' + this.config.adapterport)) {
						...
						if (duplicate) {
							...
							if (InterimSolutionForDeletionOfDuplicates) {
								...
								const deleteUrl = `http://${this.config.birdip}/bha-api/favorites.cgi?http-user=${this.config.birduser}&http-password=${this.birdpw}&action=remove&type=http&id=${key}`;
